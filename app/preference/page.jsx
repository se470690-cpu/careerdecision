...
export default function Preference() {
  ...
  const bars = [['직무 적합도', rv.jobFit], ['성장 가능성', rv.growth], ['통근 편의성', rv.commute], ['워라밸', rv.worklife], ['도메인 관심', s.inds.length ? 75 : 50]];
  ...
  return (
    <>
      <Stepper />
      <PageHead crumb="09 나의 선택 기준" title="내가 말한 기준과 실제 선택한 기준" desc="입력한 조건과, 인터뷰·저장·제외 행동에서 AI가 읽어낸 기준을 나란히 보여드려요." />
      <section className="split2">
        ...
        <article className="card pad-l">
          ...
          {done ? bars.map(([t, v]) => <Bar key={t} value={v} label={t} right={v >= 70 ? '높음' : v >= 50 ? '중간' : '낮음'} />) : <p className="sub">선택 인터뷰를 마치면 여기에 기준이 나타나요. <Link className="lnk" href="/interview">인터뷰하러 가기</Link></p>}
        </article>
      </section>
      <section className="split2">
      ...
      <section className="card pad-l">
        <p className="k">쓸수록 나를 더 잘 이해해요 (Learn From Me)</p>
        <div className="grid3 tight">
          ...
          <div className="mini"><small>인터뷰 답변</small><b>{state.iv.answers.length}/3</b></div>
        </div>
        <p className="note">{topC ? `저장한 공고에서 “${CONCEPTS[topC[0]].n}” 관련 업무가 반복적으로 나타났어요. 이 성향을 다음 추천에 반영할 수 있어요.` : '공고를 저장하거나 관심 없음으로 숨기면, 반복되는 패턴을 여기에 요약해 드려요.'}</p>
