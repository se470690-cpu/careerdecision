...
export function explainDecision(o, list) {
  ...
  let judge = `지금까지 고른 걸 보면 ${DIM[Object.entries(o.contrib).sort((a, b2) => b2[1] - a[1])[0][0]]}를 가장 중요하게 봤어요. 그 기준으로는 ${o.c.name}의 “${o.p.title.split(' - ')[0]}” 자리를 먼저 살펴보시는 게 좋아 보여요.`;
  if (list && list[1]) {
    let bestK = null, gap = -1;
    Object.keys(DIM).forEach((k) => { const d = o.contrib[k] - list[1].contrib[k]; if (d > gap) { gap = d; bestK = k; } });
    if (bestK && gap > 0.005) judge += ` 2순위 “${list[1].p.title.split(' - ')[0]}”보다 ${DIM[bestK]} 쪽에서 앞서요.`;
  }
  return { items, judge };
}
