...
export function computeAll({ setup, resume, W, scope, workFlex, hidden = [] }, postings) {
  ...
  for (const p of postings) {
    ...
    const min = commuteMin(km);
    if (min > setup.maxMin) { funnel.life++; ex.push({ p, c, km, min, kind: 'life', why: `설정한 최대 통근시간 ${setup.maxMin}분을 약 ${min - setup.maxMin}분 넘어요. (추정 ${min}분)` }); continue; }
    ...
    ok.push({ p, c, km, min, days, adjacent, unknownDays: c.office == null, sc, career: sc.career, life, pref, total, contrib: { career: (W.career * sc.career) / wsum, life: (W.life * life) / wsum, pref: (W.pref * pref) / wsum } });
  }
...
export function explainDecision(o, list) {
  ...
  items.push({ ok: true, t: `편도 약 ${o.min}분(추정), ${o.unknownDays ? '출근 정책은 확인되지 않아 주 5일로 가정했어요' : `주 ${o.days}회 출근이에요`}.` });
